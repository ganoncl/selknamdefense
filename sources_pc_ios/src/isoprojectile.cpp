...
{	
	...
	cannonRenderData = TextureHelper::GetRenderData(TextureHelper::GetTextureNamed("ship_bullet"));
	
	impact_time.SetTimeLimit(25);
...
void IsoProjectile::InitTarget(IsoPawn* target)
{
	...
	objMode = ObjectiveMode_IsoPawn;
}

void IsoProjectile::InitTarget(IsoTerrain* target)
{
	//Sanity Check
	if(target == NULL)
	{
	...
	objMode = ObjectiveMode_IsoTerrain;
}
...
void IsoProjectile::InitIso()
{
	...
	{
		impact_tex = NULL;
		setWidth(26 * globalScale);
		setHeight(5 * globalScale);
	...
	else if(type == Projectile_Rock)
	{
		...
		setWidth(12 * globalScale);
		setHeight(12 * globalScale);
		angle = 0;
		InitCastelJauAlgorithm(Mode_Parabolic);
	}
...
void IsoProjectile::SetAttackRange(int range)
{
	splashAttackRange = range;
}

void IsoProjectile::Update()
{
	/*
	if(bFailedToInit || target == NULL)
		return;
	*/
	impact_time.Update();
	...
	{
		if(objMode == ObjectiveMode_IsoPawn)
		{
			if(target != NULL)
			{
		...
		else if(objMode == ObjectiveMode_IsoTerrain)
		{
			if(terrainTarget != NULL)
			{
				int minX = terrainTarget->getXNumTile() - splashAttackRange;
				int maxX = terrainTarget->getXNumTile() + splashAttackRange;
				int minY = terrainTarget->getYNumTile() - splashAttackRange;
				int maxY = terrainTarget->getYNumTile() + splashAttackRange;

				if(minX < 0) minX = 0;
				if(minY < 0) minY = 0;
				if(maxX >= MAX_TILESROW) maxX = MAX_TILESROW - 1;
				if(maxY >= MAX_TILESROW) maxY = MAX_TILESROW - 1;

				int x_tile_me = terrainTarget->getXNumTile();
				int y_tile_me = terrainTarget->getYNumTile();
				...
				if(bToEnemyDamage)
				{
					...
					{
						IsoEnemy* tmp = &game->isoEnemyArr[i];
						if(tmp->getInitialized() && tmp->GetEnemyType() != ENEMY_None && tmp->GetEnemyType() != ENEMY_Captain)
						{
							int x_tile = Utils::Round(tmp->getIsoPosition().x / 100);
							int y_tile = Utils::Round(tmp->getIsoPosition().y / 100);
							if(abs(x_tile - x_tile_me) <= splashAttackRange && abs(y_tile - y_tile_me) <= splashAttackRange)
							{
								tmp->TakeDamage(damage, inflictor, inflictor_att, skill);
								for(int j = 0; j < MAX_ISO_ENEMIES; j++)
								{
									if(game->isoExplotion[j].index == -1)
									{

										game->isoExplotion[j].index = 0;
										game->isoExplotion[j]._x = tmp->getPosX();
										game->isoExplotion[j]._y = tmp->getPosY() + 0.01f;
										game->isoExplotion[j]._z = tmp->getPosZ();
										game->isoExplotion[j].timer.Reset();
										game->isoExplotion[j].timer.Start();
										//tmp->SetIsoExplotionIndex(j);
				...
				{
					IsoSelknam* tmp = &game->isoSelknamArr[i];
					if(tmp->getInitialized() && tmp->GetSelknamType() != SELKNAM_None)
					{
						int x_tile = Utils::Round(tmp->getIsoPosition().x / 100);
						int y_tile = Utils::Round(tmp->getIsoPosition().y / 100);
						if(abs(x_tile - x_tile_me) <= splashAttackRange && abs(y_tile - y_tile_me) <= splashAttackRange)
				...
				for(int j = minY; j <= maxY; j++)
				{
					...
					{
						if(game->isoTerrainAstarGrid[i][j] != NULL && game->isoTerrainAstarGrid[i][j]->CurrentIsoSelknam == NULL)
						{
							for(int k = 0; k < MAX_ISO_ENEMIES; k++)
							{
								if(game->isoExplotion[k].index == -1)
								{
									game->isoExplotion[k].index = 0;
									game->isoExplotion[k]._x = game->isoTerrainAstarGrid[i][j]->getPosX();
									game->isoExplotion[k]._y = game->isoTerrainAstarGrid[i][j]->getPosY() - 16.0f;
									game->isoExplotion[k]._z = game->isoTerrainAstarGrid[i][j]->getPosZ();
									game->isoExplotion[k].timer.Reset();
									game->isoExplotion[k].timer.Start();
									game->isoExplotion[k].bRender = true;
									break;
								}
							}
						}
		...
		else if(objMode == ObjectiveMode_IsoTerrain)
		{
			if(terrainTarget != NULL)
			{
				impact_position.x = int((rand() % 32) - 16 + terrainTarget->getPosX()); 
				impact_position.y = int(-28 + terrainTarget->getPosY());
			}
		}
	}
}

void IsoProjectile::RangeAttackAffectsToEnemies(bool value)
{
	bToEnemyDamage = true;
}
...
{	
	...
	{
		uvMinX = cannonRenderData.uvInfo.minX;
		uvMinY = cannonRenderData.uvInfo.minY;
		uvMaxX = cannonRenderData.uvInfo.maxX;
		uvMaxY = cannonRenderData.uvInfo.maxY;
	}
...
void IsoProjectile::RenderImpact()
{
	if(bImpact)
	{
		if(impact_tex != NULL) {
...
{
	if(objMode == ObjectiveMode_IsoPawn && (target == NULL || target->GetTile() == NULL))
	{
		bFailedToInit = true;
		return;
	}
	...
	int target_diff = 0;
	int inflictor_diff = 0;

	if(objMode == ObjectiveMode_IsoTerrain)
	{
		target_diff    = abs(terrainTarget->getXNumTile() - inflictor->GetTile()->getXNumTile());
		inflictor_diff = abs(terrainTarget->getYNumTile() - inflictor->GetTile()->getYNumTile());
	}
	else if(objMode == ObjectiveMode_IsoPawn)
	{
		target_diff    = abs(target->GetTile()->getXNumTile() - inflictor->GetTile()->getXNumTile());
		inflictor_diff = abs(target->GetTile()->getYNumTile() - inflictor->GetTile()->getYNumTile());
	}
	...
	{	
		...
		if(type == Projectile_Cannon && terrainTarget != NULL)
		{
			if(dir == DIR_South)
			{
				//IwTrace(BOSSBATTLE, (""));
				setPosZ(terrainTarget->getPosZ() + 0.001f);
			}
		}
	...
	if(type == Projectile_Cannon)
	{
		speed = (0.085f * 145.0f * globalScale) / float(fabs(points[0].x - points[length_points - 1].x) + fabs(points[0].y - points[length_points - 1].y));
	}
	else
	{
		speed = (0.085f * 180.0f * globalScale) / float(fabs(points[0].x - points[length_points - 1].x) + fabs(points[0].y - points[length_points - 1].y));
	}
	speed = speed * FPSMULTIPLIER * currentGameSpeedValue;
...
bool IsoProjectile::NotTooClose() 
{
	...
	{
		if(fabs(terrainTarget->getPosX() - getPosX()) <= 16 && fabs(terrainTarget->getPosY() - getPosY()) <= 8)
			return true;
		else
			return false;
	}
