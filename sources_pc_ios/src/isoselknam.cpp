...
void IsoSelknam::CleanIso()
{
	...
	InitialTile = NULL;
	bFixPawnOrientation = false;
	//bSellUnit = false;

	lastEnemyTypeTargeted = ENEMY_None;
}
...
void IsoSelknam::InitIsoPawn(IsoTerrain* _tile)
{
	...
	SetOrientationTowardsNearestEnemySpot();
}
...
void IsoSelknam::Update()
{
	...
	if(bSellUnit)
	{
		if(isoCoin_idx == -1)
		{
			GetCurrentTile()->CurrentIsoSelknam = NULL;
			GetCurrentTile()->aStar_aditionalCost = 0;
			CleanSpawnTile();
			CleanIso();
		}
	}
	*/
	...
	{
		if(CheckIfCanStop() || (GetTile() != NULL && GetTile()->getIsRamp()))
		{
...
void IsoSelknam::InitSelknam(IsoSelknamType _selknamType, bool bDeploySound, int overrideLevel)
{
	int myLevel = 0;
	this->SelknamType = _selknamType;
	if(SelknamType == SELKNAM_Archer_Base)
	{
		myLevel = ArcherBaseUpgrades.level - 1;
		if(overrideLevel > 0)
			myLevel = overrideLevel - 1;
		SetPawnAtributes(SelknamAtt_Archer_Base[myLevel]);
	...
	{
		myLevel = ArcherPoisonerUpgrades.level - 1;
		if(overrideLevel > 0)
			myLevel = overrideLevel - 1;
		SetPawnAtributes(SelknamAtt_Archer_Poisoner[myLevel]);
	...
	{
		myLevel = ArcherRetarderUpgrades.level - 1;
		if(overrideLevel > 0)
			myLevel = overrideLevel - 1;
		SetPawnAtributes(SelknamAtt_Archer_Retarder[myLevel]);
	...
	{
		myLevel = ShamanHealerUpgrades.level - 1;
		if(overrideLevel > 0)
			myLevel = overrideLevel - 1;
		SetPawnAtributes(SelknamAtt_Shaman_Healer[myLevel]);
	...
	{
		myLevel = ShamanMightUpgrades.level - 1;
		if(overrideLevel > 0)
			myLevel = overrideLevel - 1;
		SetPawnAtributes(SelknamAtt_Shaman_Might[myLevel]);
	...
	{
		myLevel = ShamanHasteUpgrades.level - 1;
		if(overrideLevel > 0)
			myLevel = overrideLevel - 1;
		SetPawnAtributes(SelknamAtt_Shaman_Haste[myLevel]);
	...
	{
		myLevel = WarriorBaseUpgrades.level - 1;
		if(overrideLevel > 0)
			myLevel = overrideLevel - 1;
		this->SetPawnAtributes(SelknamAtt_Warrior_Base[myLevel]);
	...
	{
		myLevel = WarriorTankUpgrades.level - 1;
		if(overrideLevel > 0)
			myLevel = overrideLevel - 1;
		this->SetPawnAtributes(SelknamAtt_Warrior_Tank[myLevel]);
	...
	{
		myLevel = WarriorBerserkUpgrades.level - 1;
		if(overrideLevel > 0)
			myLevel = overrideLevel - 1;
		this->SetPawnAtributes(SelknamAtt_Warrior_Berserk[myLevel]);
	...
	{
		myLevel = BowlerStunerUpgrades.level - 1;
		if(overrideLevel > 0)
			myLevel = overrideLevel - 1;
		this->SetPawnAtributes(SelknamAtt_Bowler_Stuner[myLevel]);
	...
	{
		myLevel = BowlerRooterUpgrades.level - 1;
		if(overrideLevel > 0)
			myLevel = overrideLevel - 1;
		this->SetPawnAtributes(SelknamAtt_Bowler_Rooter[myLevel]);
	...
	{
		myLevel = BowlerConfuserUpgrades.level - 1;
		if(overrideLevel > 0)
			myLevel = overrideLevel - 1;
		this->SetPawnAtributes(SelknamAtt_Bowler_Confuser[myLevel]);
...
void IsoSelknam::DeploySound(int soundType)
{
	//handle deploy sound
	int deploy_sound = rand() % 3;
	if(soundType==0)
	{
		if(deploy_sound==0){Utils::PlaySoundFX(FX_Selknam_Archer_Deploy_01);}
		else if(deploy_sound==1){Utils::PlaySoundFX(FX_Selknam_Archer_Deploy_02);}
		else if(deploy_sound==2){Utils::PlaySoundFX(FX_Selknam_Archer_Select_01);}
	}
	else if(soundType==1)
	{
		if(deploy_sound==0){Utils::PlaySoundFX(FX_Selknam_Mage_Deploy_01);}
		else if(deploy_sound==1){Utils::PlaySoundFX(FX_Selknam_Mage_Deploy_02);}
		else if(deploy_sound==2){Utils::PlaySoundFX(FX_Selknam_Mage_Select_01);}
	}
	else if(soundType==2)
	{
		if(deploy_sound==0){Utils::PlaySoundFX(FX_Selknam_Warrior_Deploy_01);}
		else if(deploy_sound==1){Utils::PlaySoundFX(FX_Selknam_Warrior_Deploy_02);}
		else if(deploy_sound==2){Utils::PlaySoundFX(FX_Selknam_Warrior_Select_01);}
	}
	else if(soundType==3)
	{
		if(deploy_sound==0){Utils::PlaySoundFX(FX_Selknam_Bowler_Deploy_01);}
		else if(deploy_sound==1){Utils::PlaySoundFX(FX_Selknam_Bowler_Deploy_02);}
		else if(deploy_sound==2){Utils::PlaySoundFX(FX_Selknam_Bowler_Select_01);}
	}
}
...
void IsoSelknam::SetAttackSpeed()
{
	...
	animations.UpdateAnimExtraTime("AttackNorth",     (int)((10 - attackspeed - attackspeed_modifier)));
	animations.UpdateAnimExtraTime("AttackSouth",     (int)((10 - attackspeed - attackspeed_modifier)));
	animations.UpdateAnimExtraTime("AttackEast",      (int)((10 - attackspeed - attackspeed_modifier)));
	animations.UpdateAnimExtraTime("AttackWest",      (int)((10 - attackspeed - attackspeed_modifier)));
	animations.UpdateAnimExtraTime("AttackNorthEast", (int)((10 - attackspeed - attackspeed_modifier)));
	animations.UpdateAnimExtraTime("AttackNorthWest", (int)((10 - attackspeed - attackspeed_modifier)));
	animations.UpdateAnimExtraTime("AttackSouthWest", (int)((10 - attackspeed - attackspeed_modifier)));
	animations.UpdateAnimExtraTime("AttackSouthEast", (int)((10 - attackspeed - attackspeed_modifier)));
...
void IsoSelknam::MoveToIsoTerrain(IsoTerrain* _tileFinal, bool bPermanent)
{
	...
	//IwTrace(TABLET, ("_tileFinal: %d %d", _tileFinal->getXNumTile(), _tileFinal->getYNumTile()));
	//IwTrace(TABLET, ("CurrentTile: %d %d", CurrentTile->getXNumTile(), CurrentTile->getYNumTile()));

	bMove = aStarHelperRef->AStarRetrievePathByGrid(gameRef->isoTerrainAstarGrid, CurrentTile, _tileFinal, currentIsoPath);

	//if(bMove) IwTrace(TABLET, ("bMove = true"));
	//else IwTrace(TABLET, ("bMove = false"));
...
IsoPawn* IsoSelknam::SearchEnemyByDebuff(Skill_Type skill)
{
	...
	for(int i = 0; i < MAX_ISO_ENEMIES; i++)
	{
		...
		{
			lastEnemyTypeTargeted = CurrentIsoEnemy->GetEnemyType();
			if(IsPawnInRange(CurrentIsoEnemy, &curr_distance)) {

				if(CurrentIsoEnemy->GetEnemyType() == ENEMY_Captain) return CurrentIsoEnemy;
...
IsoPawn* IsoSelknam::SearchPawnNearby()
{
	...
	for(int i = 0; i < MAX_ISO_ENEMIES; i++)
	{
		...
		if(CurrentIsoEnemy->GetEnemyType() != ENEMY_None && !CurrentIsoEnemy->GetIsDead())
		{
			...
			if(IsPawnInRange(CurrentIsoEnemy, &curr_distance)) {
				//Boss priorization.
				if(sBehavior == BHS_Range_SingleFocus && CurrentIsoEnemy->GetEnemyType() == ENEMY_Captain)
				{
					return CurrentIsoEnemy;
				}
...
IsoPawn* IsoSelknam::SearchAlliesNearby()
{
	...
	//if there no allies chek it self
	if(CloserAlly == NULL)
	{
		for(int i = 0; i < MAX_ISO_SELKNAM; i++)
		{
...
bool IsoSelknam::IsPawnInRange(IsoPawn* enemy, int* distance, int altRange)
{
	...
	if(lastEnemyTypeTargeted == ENEMY_Captain && captainBoss->CanBeHit())
	{
...
void IsoSelknam::NoPawnInteraction()
{
	...
	if(sBehavior == BHS_Melee_Ofensive_Defensive)
	{
		...
		if(pawnTargeted == NULL)
		{
			...
			if(GetTile() == spawn_tile) 
			{
				//Idle in his tile waiting for an enemy, aims to the nearest enemy camp.
				if(bFixPawnOrientation)
				{
					SetOrientationTowardsNearestEnemySpot();
				}
	...
	for(int i = 0; i < MAX_ISO_PROJECTILE; i++)
	{
		if(gameRef->isoProjectileArr[i].GetType() == Projectile_None)
		{
			   ...
			   SelknamType == SELKNAM_Archer_Retarder)
			{
				...
				int arrow_shoot_sound = rand() % 2;
				if(arrow_shoot_sound==0){Utils::PlaySoundFX(FX_Arrow_Shooting_01);}
				else if(arrow_shoot_sound==1){Utils::PlaySoundFX(FX_Arrow_Shooting_02);}
...
void IsoSelknam::CheckIsCanDoDamageByAnimation()
{
	...
	if(Utils::CanAttackByAnimation(SelknamType, animations.GetCurrentAnimID()) && DoAttackOnce)
	{
		...
		if(SelknamType == SELKNAM_Warrior_Base || SelknamType == SELKNAM_Warrior_Tank || SelknamType == SELKNAM_Warrior_Berserk) 
		{
			//Utils::PlayFXSound(FX_Sound_KNIFESWORD01);
			int punch_sound = rand() % 5;

			if(punch_sound==0){Utils::PlaySoundFX(FX_Punch_01);}
			else if(punch_sound==1){Utils::PlaySoundFX(FX_Punch_02);}
			else if(punch_sound==2){Utils::PlaySoundFX(FX_Punch_03);}
			else if(punch_sound==3){Utils::PlaySoundFX(FX_Punch_04);}
			else if(punch_sound==4){Utils::PlaySoundFX(FX_Punch_05);}
		}
	}
}
					...
					for(int j = tile_y_enemy - 1; j <= tile_y_enemy + 1; j++)
					{
						if(i >= 0 && j >= 0 && i < MAX_TILESROW && j < MAX_TILESROW)
						{
							if(gameRef->isoTerrainAstarGrid[i][j] != NULL)
							{
								...
								if((gameRef->isoTerrainAstarGrid[i][j]->getLayer() + 1) == enemy->getLayer() || (enemy->GetTile() != NULL && enemy->GetTile()->getIsRamp()))
								{
									int dx = abs(tile_x - i);
									int dy = abs(tile_y - j);
									if(dxi > dx || dyi > dy)
...
bool IsoSelknam::CheckIfCanAttack(IsoPawn* enemy)
{
	if(sBehavior == BHS_Melee_Ofensive_Defensive || sBehavior == BHS_Melee_Defensive)
	{
		...
		{
			if(enemy->GetTile() != NULL && enemy->GetTile()->getIsRamp())
			{
				return true;
			}
			else
...
void IsoSelknam::HandleDie()
{
	...
	//Handle Selknam Dying Voice
	if(SelknamType == SELKNAM_Archer_Base || SelknamType == SELKNAM_Archer_Poisoner || SelknamType == SELKNAM_Archer_Retarder)
	{
		Utils::PlaySoundFX(FX_Dying_Selknam_Archer);
	}
	else if(SelknamType == SELKNAM_Bowler_Stuner || SelknamType == SELKNAM_Bowler_Confuser || SelknamType == SELKNAM_Bowler_Rooter)
	{
		Utils::PlaySoundFX(FX_Dying_Selknam_Bowler);
	}
	else if(SelknamType == SELKNAM_Shaman_Haste || SelknamType == SELKNAM_Shaman_Might || SelknamType == SELKNAM_Shaman_Healer)
	{
		Utils::PlaySoundFX(FX_Dying_Selknam_Mage);
	}
	else if(SelknamType == SELKNAM_Warrior_Base || SelknamType == SELKNAM_Warrior_Berserk || SelknamType == SELKNAM_Warrior_Tank)
	{
		Utils::PlaySoundFX(FX_Dying_Selknam_Warrior);
	}
...
void IsoSelknam::SetOrientationTowardsNearestEnemySpot()
{
	if(gameRef == NULL)
		return;
	
	IsoTerrain* nearestTile = NULL;
	IsoTerrain* selknamTile = GetCurrentTile();
	...
	int minManhattanDistance = INT_MAX; //infinite.

	for(int i = 0; i < MAX_INITNODES; i++)
	{
		IsoTerrain* currTile = gameRef->initNodes[i];

		if(currTile != NULL)
		{
			int currManhattanDistance = Utils::GetManhattanDistance(currTile, selknamTile);
	...
	if(nearestTile != NULL)
	{
		int diffX = nearestTile->getXNumTile() - selknamTile->getXNumTile();
		int diffY = nearestTile->getYNumTile() - selknamTile->getYNumTile();

		int maxTolerance = 2;

		if(nearestTile->getXNumTile() == 0)
		{
			IwTrace(TESTINGANGLE, ("X = 0"));
			if(diffY > maxTolerance)
			{
				direction = DIR_South;
			}
			else if(diffY < -maxTolerance)
			{
				direction = DIR_North;
			}
			else
		...
		else if(nearestTile->getXNumTile() == 17)
		{
			if(diffY > maxTolerance)
			{
				direction = DIR_South;
		...
		else if(nearestTile->getYNumTile() == 0)
		{			
			if(diffX > maxTolerance)
			{
				direction = DIR_West;
			}
			else if(diffX < -maxTolerance)
			{
				direction = DIR_East;
			}
			else
		...
		else if(nearestTile->getYNumTile() == 17)
		{			
			if(diffX > maxTolerance)
			{
				direction = DIR_East;
