...
IsoPawn::IsoPawn()
{
	...
	timerConfusionEffect.SetTimeLimit(75);
	takeDamageEffectTimer.SetTimeLimit(90);
	lastID = currID = 1;
	bIsVulnerable = true;
...
void IsoPawn::InitIsoPawn(IsoTerrain* _tile)
{
	...
	anim = ANIM_Idle;
	spawn_tile = _tile;
	bReceivedOnlyPoisonedProyectiles = 0;
	bCheckStunned = false;
	bCheckConfused = false;
	bCheckRooted = false;
	counterRecalcPath = 0;
}
...
void IsoPawn::CleanIso()
{
	...
	poison_num_interval = 0;
	poison_damage = 0;
	poison_ondeath_interval_time.Reset();
	poison_ondeath_num_interval = 0;
	poison_ondeath_damage = 0;
	slow_time.Reset();
	stun_time.Reset();
	...
	timerCountdownDeath.Reset();

	takeDamageEffectTimer.Reset();
...
float IsoPawn::getSpeed()
{
	...
	mySpeed = mySpeed * 0.1f * currentGameSpeedValue * FPSMULTIPLIER;
...
void IsoPawn::SetPawnAtributes(PawnAttributes _newPawnAtributes)
{
	...
	speed = (int)(_newPawnAtributes.speed * globalScale);
...
void IsoPawn::SetUV(int id)
{
	...
	/*
	int id_x = id % horizontalImages;
	int id_y = (int)(id / horizontalImages);

	this->setUvMaxX(uvWidth * (id_x + 1));
	this->setUvMinX(uvWidth * id_x);
	this->setUvMaxY(uvHeight * (id_y + 1));
	this->setUvMinY(uvHeight * id_y);
	*/
}

void IsoPawn::SetAttackAnim(const char* anim_name)
{
	const char* myAnimName = animations.GetCurrenAnimName();
	if(strcmp(anim_name, myAnimName) != 0)
...
void IsoPawn::HandleDie()
{
	...
	//Achievement 9 check poison only death, increment counter.
	if(bReceivedOnlyPoisonedProyectiles == 1)
	{
		gameRef->bDeathOnlyByPoison++;
		if(gameRef->bDeathOnlyByPoison == 6)
		{
			//Achievement nº 9.
			//Super poisoner: matar más de 5 unidades usando flechas envenenadas.	
			achievementsHelper->SetAchievementCompletedById(8);
		}
	}
	bReceivedOnlyPoisonedProyectiles = 0;
}
...
void IsoPawn::VerifyDebuffStatus()
{
	...
	if(has_skill_effect[(int)Skill_Type_Poison])
	{
		...
		{
			TakeDamage(poison_damage, Skill_Type_Poison);
			poison_num_interval--;
	...
	if(has_skill_effect[(int)Skill_Type_CountdownDeath])
	{
		timerCountdownDeath.Update();
		if(timerCountdownDeath.GetIsEnd())
		{
			has_skill_effect[(int)Skill_Type_CountdownDeath] = false;
			TakeDamage(10000);
		}
	}
...
void IsoPawn::TakeDamage(int damage, IsoPawn* inflictor, PawnAttributes inflictor_att, Skill_Type skill_type)
{
	...
	else if(skill_type == Skill_Type_Slow && !has_skill_effect[(int)Skill_Type_Slow])
	{
		...
		has_skill_effect[(int)skill_type] = true;
		speed_modifier = (int)(Skill_Att_Slow[inflictor_att.level - 1].debuff_slow_amount * 0.75f);
	...
	else if(skill_type == Skill_Type_CountdownDeath && !has_skill_effect[(int)Skill_Type_CountdownDeath])
	{
		int prob = rand() % 100;
		if(prob <= countDownDeathDebuff_Chance)
		{
			last_debuff = skill_type;
			has_skill_effect[(int)skill_type] = true;
			timerCountdownDeath.SetTimeLimit(finalboss_deathdebuff_time);
			timerCountdownDeath.Reset();
			timerCountdownDeath.Start();
		}
	}
	...
	//Achievement 9 check poison only death
	if(sBehavior == BHS_None && eBehavior != BHE_None && has_skill_effect[(int)Skill_Type_Poison] && skill_type == Skill_Type_Poison
	&& (bReceivedOnlyPoisonedProyectiles == 0 || bReceivedOnlyPoisonedProyectiles == 1))
	{
		IwTrace(STEAMACHIEVEMENTS, ("TakeDamage(skilltype) called, PoisonCheck yes"));
		bReceivedOnlyPoisonedProyectiles = 1;
	}
	else if(sBehavior == BHS_None && eBehavior != BHE_None)
	{
		IwTrace(STEAMACHIEVEMENTS, ("TakeDamage(skilltype) called, PoisonCheck disabled"));
		bReceivedOnlyPoisonedProyectiles = 2;
	}
...
void IsoPawn::CheckDebuffedAchievement()
{
	if(sBehavior == BHS_None && eBehavior != BHE_None && bCheckStunned && bCheckConfused && bCheckRooted)
	{
		//Achievement nº 10.
		//Debuffer: Confundir, atontar y atrapar con la red a un mismo enemigo.
		achievementsHelper->SetAchievementCompletedById(9);
		bCheckStunned = false;
		bCheckConfused = false;
		bCheckRooted = false;
	}
...
void IsoPawn::RemoveCountdownDeathDebuff()
{
	has_skill_effect[(int)Skill_Type_CountdownDeath] = false;
	timerCountdownDeath.Stop();
}

void IsoPawn::SetCurrentHealth(int health)
{
	this->health = health;
}

void IsoPawn::SetCurrentMaxHealth(int maxHealth)
{
	this->healthMax = maxHealth;
}

int IsoPawn::GetCountdownDeathTimeLeft()
{
	return timerCountdownDeath.GetCurrTime_Reversed();
}


bool IsoPawn::IsDebuffed()
{
	return has_skill_effect[(int)Skill_Type_Slow] 
	       || has_skill_effect[(int)Skill_Type_PoisonOnDeath]
		   || has_skill_effect[(int)Skill_Type_Poison];
}
...
void IsoPawn::TakeDamage(int damage, Skill_Type skill_type)
{
	if(bDead)
		return;
	...
	//Achievement 9 check poison only death
	if(sBehavior == BHS_None && eBehavior != BHE_None && has_skill_effect[(int)Skill_Type_Poison] && bReceivedOnlyPoisonedProyectiles == 0)
	{
		IwTrace(STEAMACHIEVEMENTS, ("TakeDamage() called, PoisonCheck yes"));
		bReceivedOnlyPoisonedProyectiles = 1;
	}
	else if(sBehavior == BHS_None && eBehavior != BHE_None && skill_type != Skill_Type_Poison)
	{
		IwTrace(STEAMACHIEVEMENTS, ("TakeDamage() called, PoisonCheck disabled"));
		bReceivedOnlyPoisonedProyectiles = 2;
	}
...
void IsoPawn::PlayTakeDamageEffect()
{
	takeDamageEffectTimer.Reset();
	takeDamageEffectTimer.Start();
}
...
bool IsoPawn::CheckIfCanStop()
{
	...
	*/
	...
	//IwTrace(TABLE, ("%d %d => %d %d", (int)getIsoPosition().x, (int)getIsoPosition().y, tmpX, tmpY));
	//return true;
	bool bTemp = false;
	int diffX = abs(tmpX % 100 - 10);
	int diffY = abs(tmpY % 100 - 20);
	if(diffX <= 2 && diffY <= 2) {
		bRampWalked = false;
		return true;
	}
...
void IsoPawn::UnLockTerrain(IsoTerrain* tile)
{
	if(tile != NULL && !tile->getIsRamp())
	{
		...
		//{
			...
			//}
			if(tile->GetCanPutUnitInitValue())
				tile->setCanPutUnit(true);
		//}
	}
