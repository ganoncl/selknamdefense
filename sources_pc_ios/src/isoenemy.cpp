...
void IsoEnemy::CleanIso()
{
	...
	for(int i = 0; i < LAST_TILES_ARR_SIZE; i++)
	{
		lastTiles[i] = make_pair(-1, -1);
	}
...
void IsoEnemy::InitIsoPawn(IsoTerrain* _tile)
{
	...
	else if(enemyType == ENEMY_Captain)
	{
		healthMax = finalboss_max_health;
		offsetX = 0;
		offsetY = -250;
		level = 4;
		coins_perkill = 0;
...
void IsoEnemy::Update()
{
	...
	{
		setPosition(captainBoss->GetPosition().x + 75, captainBoss->GetPosition().y + 25, captainBoss->GetPosition().z);
	}
...
void IsoEnemy::NoPawnInteraction()
{
	...
	if(bRecalcPath && CheckMovementConstraints() && (CheckIfCanStop() || !bMove))
	{
		...
		{
			//IwTrace(SDASDASDs, ("myTerrainLock: %d %d", myTerrainLock->getXNumTile(), myTerrainLock->getYNumTile()));
			UnLockTerrain(myTerrainLock);
			myTerrainLock = NULL;
		}
		else
		{
			//IwTrace(ASDAS, ("FUCK YOU"));
		}
...
bool IsoEnemy::ChangeTargetCondition(IsoPawn* target)
{
	...
	if(eBehavior == BHE_Melee_Ofensive_Defensive)
	{
		if(pawnTargeted != NULL && (pawnTargeted->GetIsDead() || !pawnTargeted->getRender() || !pawnTargeted->getInitialized() || !IsPawnInRange(pawnTargeted)))
		{
			counterRecalcPath++;
			//IwTrace(AFDXAFDAFDA, ("ChangeTargetCondition() called, Pawn Id: %i, counterRecalcPath %i", getPawnId(), counterRecalcPath));
			bRecalcPath = true;
			return true;//target->GetIsDead();
		}
	}
	else if(eBehavior == BHE_Range_SingleFocus)
	{
...
void IsoEnemy::MoveToIsoTerrain(IsoTerrain* _tileFinal, bool bPermanent)
{
	...
	if(gameRef->objectivesNodes[getTargetNum() - 1] == _tileFinal)
		bFollow = false;
...
void IsoEnemy::HandleDie()
{
	...
	{
		extraCoins = finalboss_extracoins_per_enemy;
	}

	gameRef->setMPCoins(gameRef->getMPCoins() + coins_perkill + extraCoins);
	gameRef->FixMaxCoins();
	gameRef->AddTimeBonusByEnemyDie();
	//Utils::PlayFXSound(FX_Sound_COINS);
	Utils::PlaySoundFX(FX_Coins);
	//IwTrace(DEATH, ("COIN"));
	UnLockTerrain(GetTile());
	...
	else if(enemyType == ENEMY_Convict)
	{
		...
		bool bSomeSelknamDies = false;
		bool bSomeSelknamTank = false;
			...
			if(target->getInitialized() && target->GetSelknamType() != SELKNAM_None)
			{
				...
				if(abs(x_tile - x_tile_me) <= explode_range && abs(y_tile - y_tile_me) <= explode_range)
				{
					...
					if(target->GetCurrentHealth()==0)
						bSomeSelknamDies = true;
					if(target->GetSelknamType()==SELKNAM_Warrior_Tank)
						bSomeSelknamTank = true;
		...
		//Achievement nº 4.
		//Honorable Tank: Aguantar una explosión suicida con un tanque y que ninguno de tus aliados muera.		
		if(!bSomeSelknamDies && bSomeSelknamTank)
			achievementsHelper->SetAchievementCompletedById(3);
	...
	if(enemyType == ENEMY_Soldier)
	{
		Utils::PlaySoundFX(FX_Dying_Soldier);
	}
	else if(enemyType == ENEMY_Varon)
	{
	...
	{
		Utils::PlaySoundFX(FX_Dying_Convict);
	}
	else if(enemyType == ENEMY_Scientist)
	{
		Utils::PlaySoundFX(FX_Dying_Scientist);
	}
	else if(enemyType == ENEMY_Tank)
	{
		Utils::PlaySoundFX(FX_Dying_Tank);
	}
	else if(enemyType == ENEMY_Rabbit)
	{
		Utils::PlaySoundFX(FX_Dying_Rabbit);
	}

	if(tutorial->currBreakpoint == Tutorial_BreakPoint_EnemyDead)
	{
...
void IsoEnemy::DoSpecificActionOnEndDebuff(Skill_Type skill)
{
	...
	if(GetEnemyType() == ENEMY_Captain && captainBoss->GetIsActive())
	{
		captainBoss->ApplyDebuffToBoss(skill);
	}
}
...
void IsoEnemy::CheckIsCanDoDamageByAnimation()
{
	if(Utils::CanAttackByAnimation(enemyType, animations.GetCurrentAnimID()) && DoAttackOnce)
	{
		...
		if(enemyType == ENEMY_Soldier || enemyType == ENEMY_Scientist || enemyType == ENEMY_Pirate) 
		{
			//Utils::PlayFXSound(FX_Sound_KNIFESWORD01);
			int knife_sound = rand() % 3;

			if(knife_sound==0){Utils::PlaySoundFX(FX_Knife_Sword_01);}
			else if(knife_sound==1){Utils::PlaySoundFX(FX_Knife_Sword_02);}
			else if(knife_sound==2){Utils::PlaySoundFX(FX_Knife_Sword_03);}
		}
...
IsoTerrain* IsoEnemy::FindClosestFreeTileTo(IsoTerrain* tile, int range_tile)
{
	...
	for(int i = tile->getXNumTile() - range_tile; i <= (tile->getXNumTile() + range_tile); i++)
	{
		for(int j = tile->getYNumTile() - range_tile; j <= (tile->getYNumTile() + range_tile); j++)
		{
			...
			if(!gameRef->isoTerrainAstarGrid[i][j]->getIsWalkeable())
				continue;
	...
	//Prevents enemies looping between two tiles. (last 3 tiles were a enemy has been placed are stored, 
	//so if an enemy wants to go to one of these three tiles, then ignore that action.
	if(eBehavior == BHE_Melee_Ofensive_Defensive)
	{
		bool bFound = false;
		...
		for(int i = 0; i < LAST_TILES_ARR_SIZE; i++)
		{
			if(closest_tile->getXNumTile() == lastTiles[i].first && closest_tile->getYNumTile() == lastTiles[i].second)
			{
				bFound = true;
				break;
			}
		...
		lastTilesCnt++;
		if(lastTilesCnt >= LAST_TILES_ARR_SIZE)
			lastTilesCnt = 0;

		lastTiles[lastTilesCnt] = make_pair(closest_tile->getXNumTile(), closest_tile->getYNumTile());
	}
...
void IsoEnemy::IsoEnemy_Offensive_Algorithm(IsoPawn* enemy)
{
	...
	{
		if((!bFollow && CheckIfCanStop() && !enemy->bMove) || (pawnTargeted != NULL && !bMove && GetTile() != follow_tile))
		{
...
void IsoEnemy::IsoEnemy_Suicidal(IsoPawn* enemy)
{
	...
	*/
	...
	else
	{
		...
		if((follow_tile == GetTile() && (IsPawnInRange_ByIsoPosition(pawnTargeted) || follow_tile->getIsRamp())) || IsPawnInRange(pawnTargeted))
		{
...
void IsoEnemy::DoSpecificActionOnTakeDamage()
{
	...
	if(GetEnemyType() == ENEMY_Tank)
	{
		...
		if(dx == 0)
			Utils::PlaySoundFX(FX_MetalImpact_01);
		else
			Utils::PlaySoundFX(FX_MetalImpact_02);
	}
}