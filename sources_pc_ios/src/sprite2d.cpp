...
Sprite2D::Sprite2D(CIwTexture* image, float xPos, float yPos, float xVel, float yVel)
{
	...
	screenW = (float)currentGameWidth;
	screenH = (float)currentGameHeight;
...
{
	position.x += velocity.x * FPSMULTIPLIER;
	position.y += velocity.y * FPSMULTIPLIER;
	position.z += velocity.z * FPSMULTIPLIER;
