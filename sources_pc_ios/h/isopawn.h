...
protected:
	...
	GameTimer attack_cooldown_timer;
	GameTimer cast_cooldown_timer;
	GameTimer timeToBanish;
	GameTimer time_per_attack;

	int GetAttackCooldown() { return (int)((attack_cooldown + (10 - attackspeed) - attackspeed_modifier)); }
	int GetCastCooldown() { return (int)((cast_cooldown + (10 - attackspeed))); }
	...
	void SetAttackAnim(const char* anim_name);
	...
	//Poison
	GameTimer poison_interval_time;
	int poison_num_interval;
	int poison_damage;
	//Poison on death
	GameTimer poison_ondeath_interval_time;
	...
	GameTimer timerStunEffect;
	GameTimer timerConfusionEffect;

	GameTimer timerCountdownDeath;

	IsoPawn* PawnTakeDamageToMe; //The Last Pawn that called to my TakeDamage method.

	GameTimer takeDamageEffectTimer;
...
public:
	...
	void setPawnId(int _pawnId) { pawnId = _pawnId; }
	...
	virtual bool IsPawnInRange(IsoPawn* pawn, int* distance = NULL, int altRange = -1);
	...
	void TakeDamage(int damage, Skill_Type skill_type = Skill_Type_None);
	...
	*/
	...
	bool PlayingTakeDamageEffect()
	{
		return !takeDamageEffectTimer.GetIsEnd() && !takeDamageEffectTimer.GetIsStop();
	}

	bool IsDebuffed();
	void RemoveCountdownDeathDebuff();
	int GetCountdownDeathTimeLeft();
	void SetCurrentHealth(int health);
	void SetCurrentMaxHealth(int maxHealth);

	bool bIsVulnerable;

	int16 bReceivedOnlyPoisonedProyectiles;
	bool bCheckStunned;
	bool bCheckConfused;
	bool bCheckRooted;

	void CheckDebuffedAchievement();
};
