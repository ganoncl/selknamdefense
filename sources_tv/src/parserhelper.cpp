...
void ParserHelper::RetrieveMapFromFile(CGame* gameRef)
{
	...
	//Reconstructing map url
	/*
	int dig = 3 - ((int)log10((double)currentMap) + 1);
	string str_map = "maps/map_";
	if(bExtraMapMode) 
	{
		IwTrace(EXTRAMAP, ("Extra Map Mode: Map %d", currentMap));
		str_map = "extramaps/extra_map_";
	} 
	else
	{
		IwTrace(EXTRAMAP, ("Campaign Mode: Map %d", currentMap));
	}
	
	for(int i = 0; i < dig; i++) 
		str_map = str_map + "0";
	int tmp_curr_map = currentMap;
	IwTrace(TRACEMAP, ("CURRENT MAP: %d", currentMap));
	while(tmp_curr_map)
	{
		int tmp_dig = tmp_curr_map % 10;
		char tmp_c = (tmp_dig + '0');
		str_map = str_map + tmp_c;
		tmp_curr_map /= 10;
	}
	str_map = str_map + ".sdm";
	IwTrace(MAPPROPERTIES_PARSER, ("MAP: %s", str_map));
	*/
	s3eFile* fileHandle = s3eFileOpen(Utils::TransformToMapFilename(currentMap, bExtraMapMode).c_str(), "rb");
	//s3eFile* fileHandle = s3eFileOpen("maps/map_001.sdm", "rb");
	if(!fileHandle)
		return;

	int32 fileSize = s3eFileGetSize(fileHandle);
	//IwTrace(SELKNAM, ("FILE SIZE: %d", fileSize));
	int dataIntToRead = 0;
	char dataByteRead = 0;
...
void ParserHelper::ReturnDataToDefault()
{
	NewGame_State = EMPTY_GAME;

	MapStatus_State[0] = CURRENT_PLAYABLE;
	for(int i = 1; i < MAX_MAPS; i++)
		MapStatus_State[i] = NOT_PLAYABLE;

	for(int i = 0; i < MAX_MAPS; i++)
		Stars_By_Level[i] = 0;

	for(int i = 0; i < MAX_EXTRA_MAPS; i++)
		Stars_By_ExtraLevel[i] = 0;
	...
	//DEMO
	ArcherBaseUpgrades.level     = 1;
	ArcherPoisonerUpgrades.level = 1;
	ArcherRetarderUpgrades.level = 1;
	BowlerStunerUpgrades.level   = 1;
	BowlerRooterUpgrades.level   = 1;
	BowlerConfuserUpgrades.level = 1;
	WarriorBaseUpgrades.level    = 1;
	WarriorTankUpgrades.level    = 1;
	WarriorBerserkUpgrades.level = 1;
	ShamanMightUpgrades.level    = 1;
	ShamanHasteUpgrades.level    = 1;
	ShamanHealerUpgrades.level   = 1;
	MapStatus_State[0] = PLAYABLE;
	MapStatus_State[1] = CURRENT_PLAYABLE;
	*/
...
void ParserHelper::SaveGameData()
{
	s3eFileDelete("data.sdm");
	s3eFileDelete(".lgapp_user_data/data.sdm");

	s3eFile* file = s3eFileOpen("data.sdm", "w+b");
	s3eFile* fileLG = s3eFileOpen(".lgapp_user_data/data.sdm", "w+b");
	...
	*/
	...
	aux = (int)NewGame_State;
	if(file != NULL) s3eFileWrite(&aux, sizeof(int), 1, file);
	if(fileLG != NULL) s3eFileWrite(&aux, sizeof(int), 1, fileLG);

	for(int i = 0; i < MAX_MAPS; i++)
	{
		aux = (int)MapStatus_State[i];
		if(file != NULL) s3eFileWrite(&aux, sizeof(int), 1, file);
		if(fileLG != NULL) s3eFileWrite(&aux, sizeof(int), 1, fileLG);
	}
	...
	for(int i = 0; i < MAX_MAPS; i++)
	{
		aux = Stars_By_Level[i];
		if(file != NULL) s3eFileWrite(&aux, sizeof(int), 1, file);
		if(fileLG != NULL) s3eFileWrite(&aux, sizeof(int), 1, fileLG);
	}

	aux = ArcherBaseUpgrades.level;
	if(file != NULL) s3eFileWrite(&aux, sizeof(int), 1, file);
	if(fileLG != NULL) s3eFileWrite(&aux, sizeof(int), 1, fileLG);
	
	aux = ArcherPoisonerUpgrades.level;
	if(file != NULL) s3eFileWrite(&aux, sizeof(int), 1, file);
	if(fileLG != NULL) s3eFileWrite(&aux, sizeof(int), 1, fileLG);

	aux = ArcherRetarderUpgrades.level;
	if(file != NULL) s3eFileWrite(&aux, sizeof(int), 1, file);
	if(fileLG != NULL) s3eFileWrite(&aux, sizeof(int), 1, fileLG);

	aux = WarriorBaseUpgrades.level;
	if(file != NULL) s3eFileWrite(&aux, sizeof(int), 1, file);
	if(fileLG != NULL) s3eFileWrite(&aux, sizeof(int), 1, fileLG);

	aux = WarriorTankUpgrades.level;
	if(file != NULL) s3eFileWrite(&aux, sizeof(int), 1, file);
	if(fileLG != NULL) s3eFileWrite(&aux, sizeof(int), 1, fileLG);

	aux = WarriorBerserkUpgrades.level;
	if(file != NULL) s3eFileWrite(&aux, sizeof(int), 1, file);
	if(fileLG != NULL) s3eFileWrite(&aux, sizeof(int), 1, fileLG);
	
	aux = ShamanHealerUpgrades.level;
	if(file != NULL) s3eFileWrite(&aux, sizeof(int), 1, file);
	if(fileLG != NULL) s3eFileWrite(&aux, sizeof(int), 1, fileLG);

	aux = ShamanMightUpgrades.level;
	if(file != NULL) s3eFileWrite(&aux, sizeof(int), 1, file);
	if(fileLG != NULL) s3eFileWrite(&aux, sizeof(int), 1, fileLG);

	aux = ShamanHasteUpgrades.level;
	if(file != NULL) s3eFileWrite(&aux, sizeof(int), 1, file);
	if(fileLG != NULL) s3eFileWrite(&aux, sizeof(int), 1, fileLG);

	aux = BowlerStunerUpgrades.level;
	if(file != NULL) s3eFileWrite(&aux, sizeof(int), 1, file);
	if(fileLG != NULL) s3eFileWrite(&aux, sizeof(int), 1, fileLG);
	
	aux = BowlerRooterUpgrades.level;
	if(file != NULL) s3eFileWrite(&aux, sizeof(int), 1, file);
	if(fileLG != NULL) s3eFileWrite(&aux, sizeof(int), 1, fileLG);
	
	aux = BowlerConfuserUpgrades.level;
	if(file != NULL) s3eFileWrite(&aux, sizeof(int), 1, file);
	if(fileLG != NULL) s3eFileWrite(&aux, sizeof(int), 1, fileLG);
	...
	for(int i = 0; i < MAX_EXTRA_MAPS; i++)
	{
		aux2 = Stars_By_ExtraLevel[i];
		if(file != NULL) s3eFileWrite(&aux2, sizeof(int8), 1, file);
		if(fileLG != NULL) s3eFileWrite(&aux2, sizeof(int8), 1, fileLG);
	}

	bool aux3 = bFirstMapWasPlayed;
	if(file != NULL) s3eFileWrite(&aux3, sizeof(bool), 1, file);
	if(fileLG != NULL) s3eFileWrite(&aux3, sizeof(bool), 1, fileLG);

	aux3 = bShowTutorialLevel1;
	if(file != NULL) s3eFileWrite(&aux3, sizeof(bool), 1, file);
	if(fileLG != NULL) s3eFileWrite(&aux3, sizeof(bool), 1, fileLG);

	aux3 = bShowTutorialStars;
	if(file != NULL) s3eFileWrite(&aux3, sizeof(bool), 1, file);
	if(fileLG != NULL) s3eFileWrite(&aux3, sizeof(bool), 1, fileLG);
...
void ParserHelper::RetrieveGameData()
{
	s3eFile* file = s3eFileOpen(".lgapp_user_data/data.sdm", "rb");

	if(file == NULL)
	{
		file = s3eFileOpen("data.sdm", "rb");
	...
	int aux;
	s3eFileRead(&aux, sizeof(int), 1, file);
	NewGame_State = (NewGame)aux;

	for(int i = 0; i < MAX_MAPS; i++)
	{
		s3eFileRead(&aux, sizeof(int), 1, file);
		MapStatus_State[i] = (MapStatus)aux;
	}

	for(int i = 0; i < MAX_MAPS; i++)
	{
		s3eFileRead(&aux, sizeof(int), 1, file);
		Stars_By_Level[i] = aux;
	}
	
	s3eFileRead(&aux, sizeof(int), 1, file);
	ArcherBaseUpgrades.level = aux;
	
	s3eFileRead(&aux, sizeof(int), 1, file);
	ArcherPoisonerUpgrades.level = aux;
	
	s3eFileRead(&aux, sizeof(int), 1, file);
	ArcherRetarderUpgrades.level = aux;
	
	s3eFileRead(&aux, sizeof(int), 1, file);
	WarriorBaseUpgrades.level = aux;

	s3eFileRead(&aux, sizeof(int), 1, file);
	WarriorTankUpgrades.level = aux;
	
	s3eFileRead(&aux, sizeof(int), 1, file);
	WarriorBerserkUpgrades.level = aux;

	s3eFileRead(&aux, sizeof(int), 1, file);
	ShamanHealerUpgrades.level = aux;
	
	s3eFileRead(&aux, sizeof(int), 1, file);
	ShamanMightUpgrades.level = aux;
	
	s3eFileRead(&aux, sizeof(int), 1, file);
	ShamanHasteUpgrades.level = aux;
	
	s3eFileRead(&aux, sizeof(int), 1, file);
	BowlerStunerUpgrades.level = aux;
	
	s3eFileRead(&aux, sizeof(int), 1, file);
	BowlerRooterUpgrades.level = aux;
	
	s3eFileRead(&aux, sizeof(int), 1, file);
	BowlerConfuserUpgrades.level = aux;
	...
	for(int i = 0; i < MAX_EXTRA_MAPS; i++)
	{
		s3eFileRead(&aux2, sizeof(int8), 1, file);
		Stars_By_ExtraLevel[i] = aux2;
	}

	bool aux3;
	s3eFileRead(&aux3, sizeof(bool), 1, file);
	bFirstMapWasPlayed = aux3;

	s3eFileRead(&aux3, sizeof(bool), 1, file);
	bShowTutorialLevel1 = aux3;

	s3eFileRead(&aux3, sizeof(bool), 1, file);
	bShowTutorialStars = aux3;

	s3eFileClose(file);
}

void ParserHelper::CheckIfSaveDataExists()
{
	IwTrace(DATA, ("Looking for save data files."));
	if(!s3eFileCheckExists("data.sdm") && (s3eFileListDirectory(".lgapp_user_data") == NULL && !s3eFileCheckExists(".lgapp_user_data/data.sdm")))
	{
		IwTrace(DATA, ("Savedata not found."));
		ReturnDataToDefault();
	}
	else
	{
		CheckSaveDataStructure();
	}
}

void ParserHelper::CheckSaveDataStructure(string filename)
{
	s3eFile* file = s3eFileOpen(filename.c_str(), "rb");

	//Fail to retrieve file, just for safe create the file again.
	if(file == NULL)
	{
		//IwTrace(DATA ("Fail to retrieve save data."));
		ReturnDataToDefault();
		return;
	}

	//Auxiliar variables to retrieve the information from the file.
	int aux;
	//We are going to try to read all the info needed to load the game properly
	//If the end of file is reached before read all the information needed, then
	//the file is corrupted and it's replaced by a new one.
	s3eFileRead(&aux, sizeof(int), 1, file);
	if(s3eFileEOF(file))
	{
		s3eFileClose(file);
...
void ParserHelper::CheckSaveDataStructure()
{
	//Check if LG persistent folder exists.
	if(s3eFileListDirectory(".lgapp_user_data") != NULL)
	{
		//If exists, then check structure of save data. If this file exists.
		CheckSaveDataStructure(".lgapp_user_data/data.sdm");
	}
	else
	{
		IwTrace(DATA, ("LG folder doesn't exists."));
		CheckSaveDataStructure("data.sdm");
	}
}